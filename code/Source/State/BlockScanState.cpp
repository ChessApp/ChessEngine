...
namespace Chess
{
  namespace State
  {
    ...
    {
      // Should never get to this state if there are no lines of check,
      // but mitigate any null ptrs by performing check here.
      if( gameState_.checkPaths.size() == 0 )
        return returnState_;

      // Blocking will not work if there are multiple lines of check.
      if( gameState_.checkPaths.size() > 1 )
        return nextState_;

      auto s = gameState_.checkPaths.front();
